...
router.get("/",(req,res)=>{
  // Copy the code here
  //send a JSON rewsponse containg the users array, formatted with an indentation of 4 spaces for readability
...
router.delete("/:email", (req, res) => {
  const email = req.params.email;
  users = users.filter((user) => user.email != email);
  //set the users array as a new array with every person whose email is not equal to the email that we passed in the argument
  res.send(`The user with the email ${email} deleted`)//This line is to be replaced with actual return value
});
